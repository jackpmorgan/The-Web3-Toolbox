...
const run = async () => {
  ...
  try {
    ...
    const tx = await sdk.deployer.deployTokenDrop({
    ...
    instance.publishState('token_drop_contract_address', tx);
    instance.triggerEvent('token_drop_contract_deployed');
  
  } catch (error) {
    console.log(error);
    const errorcode = JSON.stringify(error);
    instance.publishState('token_drop_deployment_error', errorcode);
    instance.triggerEvent('token_drop_deployment_error');
  }
