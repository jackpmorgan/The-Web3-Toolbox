...
const run = async () => {
  ...
  try {
    ...
    const tx = await sdk.deployer.deployNFTCollection({
                                name: properties.name,
                                description: properties.description,
                                image: properties.image,
                                symbol: properties.symbol,
                                seller_fee_basis_points: properties.seller_fee_basis_points,
                                fee_recipient: properties.royalty_fee_recipient,
    ...
    instance.publishState('nft_collection_contract_address', tx);
    instance.triggerEvent('nft_collection_contract_deployed');
  
  } catch (error) {
    console.log(error);
    const errorcode = JSON.stringify(error);
    instance.publishState('nft_collection_deployment_error', errorcode);
    instance.triggerEvent('nft_collection_deployment_error');
  }
